...
def load_data():
	...
	print("Minimum threshold = %.4f%%" % (100.0 * counter/len(y_to_test)))
...
print("\n------Predictions with some algorithms------")
modelOVR = OneVsRestClassifier(LinearSVC(random_state = 0))
fit(modelOVR, x_to_train, y_to_train)
resultOVR = predict(modelOVR, x_to_test, y_to_test)

modelOVO = OneVsOneClassifier(LinearSVC(random_state = 0))
fit(modelOVO, x_to_train, y_to_train)
resultOVO = predict(modelOVO, x_to_test, y_to_test)
